...
def pmlen_of_mode_forwards (arg_ : PointerMaskingMode) : SailM Int := do
  match arg_ with
  | .PMM_Disabled => (pure 0)
  | .PMM_PMLEN_7 => (pure 7)
...
/-- Type quantifiers: arg_ : Nat, arg_ ∈ {0, 7, 16} -/
def pmlen_of_mode_backwards (arg_ : Nat) : PointerMaskingMode :=
  match arg_ with
  | 0 => PMM_Disabled
  | 7 => PMM_PMLEN_7
  | _ => PMM_PMLEN_16

def pmlen_of_mode_forwards_matches (arg_ : PointerMaskingMode) : Bool :=
  match arg_ with
  | .PMM_Disabled => true
  | .PMM_PMLEN_7 => true
  | .PMM_PMLEN_16 => true
...
/-- Type quantifiers: arg_ : Nat, arg_ ∈ {0, 7, 16} -/
def pmlen_of_mode_backwards_matches (arg_ : Nat) : Bool :=
  match arg_ with
  | 0 => true
  | 7 => true
  | 16 => true
...
def get_pmlen (access : (MemoryAccessType mem_payload)) (eff_privilege : Privilege) : SailM Int := do
  ...
  then (pmlen_of_mode_forwards (← (get_pmm eff_privilege)))
  else (pure 0)
...
def get_hlsv_pmlen (access : (MemoryAccessType mem_payload)) (eff_privilege : Privilege) : SailM Int := do
  ...
  if ((← (is_pmm_applicable access eff_privilege)) : Bool)
  then (pmlen_of_mode_forwards pmm)
  else (pure 0)
